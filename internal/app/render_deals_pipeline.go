...
func renderDealsPipelineHeader(tenant control.Tenant) template.HTML {
	...
	b.WriteString(`<h1 class="text-xl lg:text-2xl font-semibold text-gray-900">Deals</h1>`)
	b.WriteString(`<span class="text-xs font-medium text-gray-500">Pipeline Lens</span>`)
	b.WriteString(`</div>`)
	b.WriteString(`</div>`)
	b.WriteString(`</div>`)
